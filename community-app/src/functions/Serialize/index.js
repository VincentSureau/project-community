// Fork made of the original form-serialize package https://github.com/defunctzombie/form-serialize
// Do some changes to be exactly the serialise that we want in this project.
...
function serialize(form, options) {
  ...
  for (let i = 0; i < elements.length; ++i) {
    ...
    if (options.empty) {
      ...
      // for image
      if (element.type === 'text' && element.value === '' && element.name.includes('/images/')) {
        continue;
      }
...
function hash_assign(result, keys, value, element) {
  if (keys.length === 0) {
      result = value;
      return result;
  }
  ...
  if (!between) {
      result[key] = hash_assign(result[key], keys, value, element);
  }
  else {
...
function hash_serializer(result, key, value, element) {
  ...
  if (matches) {
      ...
      hash_assign(result, keys, value, element);
  }
  else {
